...
def test_account_identity_is_derived_and_persisted_without_refresh(tmp_path):
    cache = seed(tmp_path / "cache", expires=time.time() + 3600)
    client = auth(cache.parent, no_network)
    assert client.get_account_id() == "test-account"
    assert json.loads(cache.read_text())["account_id"] == "test-account"
